...
# asian fixed effects don't work since all for this treatment are in california
felm_1 <- felm(turnout_4 ~ treatment + perc_fem + perc_under30 + perc_55plus |
  state | 0 | state, data = white_incumbent_treatment,
  weights = white_incumbent_treatment$group_weights)
felm_2 <- felm(turnout_4 ~ treatment + perc_fem + perc_under30 + perc_55plus |
  state | 0 | state, data = black_incumbent_treatment,
  weights = black_incumbent_treatment$group_weights)
# felm_3 <- felm(turnout_4 ~ treatment + perc_fem + perc_under30 + perc_55plus |
#   state | 0 | state, data = asian_incumbent_treatment,
#   weights = asian_incumbent_treatment$group_weights)
felm_4 <-felm(turnout_4 ~ treatment + perc_fem + perc_under30 + perc_55plus |
  state | 0 | state, data = latino_incumbent_treatment,
  weights = latino_incumbent_treatment$group_weights)

texreg::texreg(list(felm_1, felm_2, felm_3, felm_4),
  digits = 4)

individual_turnout_district_treatement <- rbind(white_district_treatment,
  black_district_treatment, latino_district_treatment)
